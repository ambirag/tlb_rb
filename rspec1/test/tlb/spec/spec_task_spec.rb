...
describe Tlb::SpecTask do
  before(:all) do
    @path_to_lib_root = File.expand_path(File.join(File.dirname(__FILE__), '..', '..', '..', 'lib', 'tlb'))
    @path_to_spec_formatter = File.expand_path(File.join(@path_to_lib_root, 'spec', 'spec_formatter'))
  end
